...
import { Shield, Users, Leaf, ArrowLeft, Send, MessageSquare, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="bg-white rounded-2xl border-2 border-blue-500 shadow-sm p-8">
      ...
      <div className="text-center mb-6">
        <p className="text-lg text-slate-600 font-medium">
          ¿Cuál cree que es el estado que mejor describe el avance de su institución en el siguiente aspecto?
        </p>
      </div>
      ...
      {currentQ ? (
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {Object.entries(currentQ.scale_descriptions).map(([score, desc]) => {
              const isSelected = responses.some(r => r.question_id === currentQ.id && r.score === parseInt(score));
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-5 border rounded-xl transition-all duration-200 ${
                    isSelected 
                      ? `${config.bg} ${config.border} border-2 ring-2 ring-opacity-20 ${config.color}`.replace('ring-purple', 'ring-purple-300').replace('ring-blue', 'ring-blue-300').replace('ring-emerald', 'ring-emerald-300')
        ...
        <div className="bg-slate-50 rounded-xl p-12 text-center mb-8">
          <p className="text-slate-500 text-lg">No hay más preguntas disponibles.</p>
        </div>
      )}

      {/* Comments Section - Only show on last question */}
      {isLastQuestion && (
